...
(() => {
  ...
  function observe(url, method, body, data) {
    ...
    if (match && method.toUpperCase() === 'POST') {
      ...
      if (
        !input ||
        typeof input.typed_code !== 'string' ||
        input.typed_code.length > 200000 ||
        !input.typed_code.trim() ||
        typeof input.lang !== 'string' ||
        !/^\d+$/.test(String(data?.submission_id))
      ) {
        return;
      }
      ...
      pending.set(String(data.submission_id), {
        ...
        slug: match[1],
        language: input.lang,
        code: input.typed_code,
        submissionId: String(data.submission_id)
      });
    }

    const check = pathname.match(
    ...
    const submission = pending.get(check[1]);
    pending.delete(check[1]);
  ...
  window.fetch = async function (input, init) {
    ...
    const watched =
      /\/(submit|check)\/?$/.test(endpoint(url));
    ...
    if (watched) {
      try {
        ...
        Promise.all([
          ...
          .then(([body, data]) => {
            observe(url, method, body, data);
          })
          .catch(() => {});
      } catch {
        /* Keep LeetCode's response intact. */
      }
    }
  ...
  XMLHttpRequest.prototype.send = function (body) {
    ...
    if (
      request &&
      /\/(submit|check)\/?$/.test(endpoint(request.url))
    ) {
      this.addEventListener(
        ...
        () => {
          try {
            ...
            /* Other response types are not submissions. */
          }
        },
        { once: true }
      );
