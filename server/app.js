...
function createServer(config, { fetchImpl = fetch } = {}) {
  ...
  const sessionFrom=req=>{try{return oauth.auth(req.headers['x-autosync-session']);}catch{return null;}};
  return http.createServer(async(req,res)=>{
    try {
      ...
      if(url.pathname==='/oauth/start'&&req.method==='GET'){
        ...
        const authUrl=oauth.begin(state,verifier);
        return json(res,200,{success:true,authorizationUrl:authUrl,state});
      }
      if(url.pathname==='/oauth/callback'&&req.method==='GET'){
        const state=url.searchParams.get('state');
        if(url.searchParams.get('error')){const message=url.searchParams.get('error_description')||'GitHub authorization was cancelled.';oauth.fail(state,message);return oauthPage(res,false,message);}
        const code=url.searchParams.get('code');
        if(!code||!state){if(state)oauth.fail(state,'Missing OAuth response.');return oauthPage(res,false,'Missing OAuth response.');}
        try{const result=await oauth.callback(code,state);return oauthPage(res,true,'GitHub connected successfully.',result.sessionId);}catch(error){oauth.fail(state,error.message);return oauthPage(res,false,error.message);}
      }
      if(url.pathname==='/oauth/poll'&&req.method==='GET') return json(res,200,{success:true,...oauth.poll(url.searchParams.get('state'))});
      ...
      if(url.pathname==='/status'&&req.method==='GET'&&session?.repository){
        const r=await githubClient({token:session.token},fetchImpl)(`/repos/${session.repository.owner}/${session.repository.name}`);if(!r.response.ok)throw githubError(r.response);
        if(r.data.permissions?.push===false)throw new SyncError('GitHub user does not have repository write access.',403);
        return json(res,200,{success:true,repository:session.repository.fullName,branch:session.repository.branch,folder:session.repository.folder,target:session.repository.target});
      }
      if(url.pathname==='/upload'&&req.method==='POST'&&session?.repository){
        ...
        const syncConfig={...config,token:session.token,owner:session.repository.owner,repo:session.repository.name,branch:session.repository.branch,folder:session.repository.folder,target:session.repository.target};
        const job=serial.then(async()=>{const result=await syncSubmission(syncConfig,payload,fetchImpl);try{await fs.mkdir(config.dataDir,{recursive:true});const log=path.join(config.dataDir,'activity.jsonl');if((await fs.stat(log).catch(()=>({size:0}))).size>2000000)await fs.rename(log,`${log}.1`);await fs.appendFile(log,JSON.stringify({at:new Date().toISOString(),slug:payload.slug,language:payload.language,...result})+'\n');}catch{result.warning='Synced, but the local activity log could not be written.';}return result;});serial=job.catch(()=>{});try{json(res,200,await job);}finally{waiting--;}
        return;
      }
      ...
      if(url.pathname==='/status'&&req.method==='GET'){
        if(!config.legacyConfigured) throw new SyncError('OAuth is required. Connect GitHub from the extension.',401);
...
function oauthPage(res,ok,message,sessionId=''){const safe=String(message).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));const payload=JSON.stringify({ok,message,sessionId});res.writeHead(ok?200:400,{'Content-Type':'text/html; charset=utf-8','Cache-Control':'no-store'});res.end(`<!doctype html><meta charset="utf-8"><title>AutoSync GitHub</title><style>body{font:16px system-ui;max-width:560px;margin:15vh auto;padding:24px}</style><h1>${ok?'GitHub connected ✓':'GitHub connection failed'}</h1><p>${safe}</p><script>const data=${payload};if(window.opener){window.opener.postMessage({source:'autosync',...data},'*');setTimeout(()=>window.close(),300)}else if(data.sessionId){document.body.insertAdjacentHTML('beforeend','<p>You can close this window and return to AutoSync.</p>')}</script>`);}
